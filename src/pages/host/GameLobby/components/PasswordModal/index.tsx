...
function PasswordModal({ roomCode }: { roomCode: string }) {
  const pwRef = useRef<HTMLDivElement>(null);
  const { closeModal, state: modalState } = useModalState();
  ...
  if (modalState('hostGameModal') !== 'password') return null;

  return (
    <ModalLayout onClick={onClickModalBackground} visible>
      <Password ref={pwRef}>
        <p>{roomCode}</p>
      </Password>
    </ModalLayout>
